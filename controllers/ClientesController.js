...
router.post('/admin/cliente/salvarEdicao', (req, res) => {
  ...
  if (!idOK) {
    idError = 'ID inválido ou preenchido de forma incorreta.' 
  }
  if (!nomeOK) {
    nomeError = 'NOME inválido ou preenchido de forma incorreta.' 
  }
  ...
  if (!identificacaoOK) {
    identificacaoError = 'IDENTIFICAÇÃO inválido ou preenchido de forma incorreta.'
  }
  ...
  // Variáveis a serem repassadas para o fron-end.
  req.flash('id', id)
  req.flash('nome', nome)
  req.flash('ano', ano)
  req.flash('email', email)
